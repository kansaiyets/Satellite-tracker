...
# 選択された衛星名を表示（デバッグ用）
st.write(f"選択された衛星: {selected_satellite}")

# 衛星データ取得
if selected_satellite in satellites_info:
    info = satellites_info[selected_satellite]
else:
    st.error(f"選択された衛星 `{selected_satellite}` は、衛星情報に存在しません。")
    st.stop()  # エラーが発生した場合は以降の処理を停止
